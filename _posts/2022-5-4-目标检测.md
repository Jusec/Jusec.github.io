...
![图片](https://s2.loli.net/2022/05/04/HPrIV1xUYXygsCo.jpg)

![image-20220504163357437](https://s2.loli.net/2022/05/04/caEntP86bTvw94Z.png)

$p_{i}$ : Anchor[i]的预测分类概率；

Anchor[i]是正样本时, $p_{i}^{*}=1$ ；Anchor[i]是负样本时， $p_{i}^{*}=0$;

$t_{i}$: Anchor[i]预测的Bounding Box的参数化坐标(parameterized coordinates)；

$t_{i}^{*}$: Anchor[i]的Ground Truth的Bounding Box的参数化坐标；

$N_{c l s}$: mini-batch size;

$N_{r e g}$: Anchor Location的数量;

$L_{r e g}: L_{r e g}\left(t_{i}, t_{i}^{*}\right)=R\left(t_{i}-t_{i}^{*}\right)$, R是Smooth L1函数；
...
### 7.faster-rcnn中bbox回归用的是什么公式，说一下该网络是怎么回归bbox的？
